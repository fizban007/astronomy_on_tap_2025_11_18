(function() {
	...
	initialize: function(canvas) {
	    ...
	    function add_animated_photon_path(theta, phi, r_i, r_f, smoothness, color) {
		var g = new THREE.Geometry();
		for (var i = 0; i < smoothness; i++) {
	    ...
	    function add_photon_path(theta, phi, r_i, r_f, smoothness, color) {
		var g = new THREE.Geometry();
		for (var i = 0; i < smoothness; i++) {
		    var r = r_i + (r_f - r_i) * i / (smoothness - 1);
		    var p = new THREE.Vector3(-r, 0, 0.08*Math.sin(2*Math.PI*r/(0.15*(r_f - r_i))));
